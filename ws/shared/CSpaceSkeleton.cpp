...
Eigen::Vector2d MyGridCSpace2D::getPointFromCell(int cellX, int cellY) const {
    ...
    // Calculate the cell index for each dimension
    double x = static_cast<double>(x0_min +(cellX + 0.5) * x0_cell_size);
    double y = static_cast<double>(x1_min + (cellY + 0.5) * x1_cell_size);
    ...
    for (std::size_t i = 0; i < num_cells_x0; ++i) {
        for (std::size_t j = 0; j < num_cells_x1; ++j) {
            ...
            for (const auto& obstacle : env.obstacles) {
                ...
                for (std::size_t k = 0; k < num_vertices; ++k) {
                    Eigen::Vector2d v1 = inflatedVertices[k];
                    Eigen::Vector2d v2 = inflatedVertices[(k + 1) % num_vertices];
